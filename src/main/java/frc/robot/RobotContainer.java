...
//     private ShootingCalculator m_shootingCalculator = new ShootingCalculator(drivetrain);
    ...
    private void configureBindings() {
        ...
        s_Hood.setDefaultCommand(new ConditionalCommand(s_Hood.goTo(()-> m_shootingParameters.calculate(s_Drivetrain.getNearestCorner().getTranslation()).hoodAngle()), 
        ...
        joystick.rightTrigger().whileTrue(new ConditionalCommand(
                Commands.parallel(s_Shooter.shoot(() -> m_shootingParameters.calculate(s_Drivetrain.getNearestCorner().getTranslation()).rpm()), s_Indexer.feed().onlyIf(s_Shooter::atSpeed).repeatedly()),
        ...
        s_Turret.setDefaultCommand(new ConditionalCommand(
                new TurretTrackPose(() -> m_shootingParameters.calculate(s_Drivetrain.getNearestCorner().getTranslation()).turretAngle().getDegrees(), () -> s_Drivetrain.getState().Pose, s_Turret),
                new TurretTrackPose(() -> m_shootingParameters.calculate(m_hub).turretAngle().getDegrees(), () -> s_Drivetrain.getState().Pose, s_Turret),
                s_Drivetrain::robotBehindHub));
